...
function openQuickView(productId) {
  ...
  overlay.querySelector('.modal__price').textContent = `R ${product.price.toFixed(2)}`;
...
function initQuickView() {
  const overlay = document.getElementById('quick-view-overlay');
  if (!overlay) return;
  overlay.querySelector('.modal__close')?.addEventListener('click', closeQuickView);
...
function initScrollReveal() {
  ...
  document.querySelectorAll('.product-card, .collection-card, .section-story').forEach(el => {
    el.style.opacity = '0';
